...
public class Library extends LinkedList<Book> {
    ...
    public Library author(String author) {
        Library authorBooks = new Library(capacity);
        for(int i = 0; i < size(); ++i)
    ...
    public Library title(String title) {
        Library titleBooks = new Library(capacity);
        for(int i = 0; i < size(); ++i)
            if(get(i)._title.equals(title))
                titleBooks.add(this.get(i));
        return titleBooks;
    }
    ...
    public double priceAverage(){
        double avg = 0.0;
        int i = 0;
        // try to do it with foreach : for(Book b : params)
        for(; i < size(); ++i)
            avg += get(i)._price;
        if (i!=0)
            avg /= i;
    ...
    public static void main(String[] args) throws Exception {
        ...
        System.out.println("Destruction de hp2for(String t : params)");
        hp2 = null;
        ...
        System.out.println("Average of prices ? " + lib.priceAverage());
    } 
}
